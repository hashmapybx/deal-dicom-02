...
path = "/media/tx-eva-data/CE_DR/原始数据库/train_data_2634"
save_path = '/media/tx-eva-data/CE_DR/原始数据库/train_data_2634'
id_list = []
KVP_list = []
...
for sfile in os.listdir(path):
    ...
    for ttfile in os.listdir(a_folder):
        if 'error' not in ttfile:
            continue
        b_folder = os.path.join(a_folder, ttfile)
        for file in os.listdir(b_folder):
            count += 1
            dcm_file_path = os.path.join(b_folder, file)
