...
def split_excel(list_old_pid):
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/Data4/CE_DATA/CE_三四级文件/数据试标记记录/seg.xls"
    ...
    for row_index in range(rows):
        if row_index == 0:
            ...
            arbitrate_doctor = sheet1.row_values(row_index)[14]
            arbitrate_time = sheet1.row_values(row_index)[15]
            ...
            file_name = mark_time + '-' + mark_project + '-' + data_type + '-' + mark_doctor1  # 生成的新的excel 命名规定
            ...
            for pid_index, pid_value in enumerate(need_mark_list):
                ...
                list_row_data.append([mark_time, mark_doctor1, mark_project, data_type, pid_value,
            ...
            workboot.save('/media/tx-eva-data/Data4/CE_DATA/CE_三四级文件/数据试标记记录/seg/%s.xls' % file_name)
...
def get_pid(mark_number, mark_project, hospital_name):
    ...
    src_path = "/media/tx-eva-data/Data4/CE_DATA/标注数据库/segmentation/dcm"
