...
root_path = "/media/tx-eva-data/NAS/原始数据库/肺部计算机辅助诊断软件/CE021008"
start = time.time()
for ssfile in os.listdir(root_path):
    ...
    for afile in os.listdir(h_folder_path):
        ...
        for bfile in os.listdir(a_folder):
            ...
            for cfile in os.listdir(b_folder):
                c_folder = os.path.join(b_folder, cfile)
                for dcm_file in os.listdir(c_folder):
                    dcm_file_path = os.path.join(c_folder, dcm_file)
                    os.system('touch -mt %s %s' % (
                    change_date_h + str(random.randint(00, 59)).zfill(2) + '.' + str(random.randint(00, 59)).zfill(2), dcm_file_path))
                os.system('touch -mt %s %s' % (
                change_date_h + str(random.randint(00, 59)).zfill(2) + '.' + str(random.randint(00, 59)).zfill(2), c_folder))
            os.system('touch -mt %s %s' % (
                change_date_h + str(random.randint(00, 59)).zfill(2) + '.' + str(random.randint(00, 59)).zfill(2),
                b_folder))
        os.system('touch -mt %s %s' % (
            change_date_h + str(random.randint(00, 59)).zfill(2) + '.' + str(random.randint(00, 59)).zfill(2),
            a_folder))
