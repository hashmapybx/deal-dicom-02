...
def write_excel():
    ...
    list_date = {}
    data = xlrd.open_workbook("/media/tx-eva-data/Data1/CFDA_file/数据脱敏/数据二次脱敏记录 .xlsx")
    # 获取每一个sheet的name?
                ...
                if date_i not in list_date:
                    ...
                    for a in range(0, len(table.row_values(i))):
                        if a == 1:
                            ...
                            sheet1.write(4, a, datetime.datetime(*xldate_as_tuple(table.row_values(i)[a], 0)).strftime('%Y-%m-%d') if a == 3 else  table.row_values(i)[a],  set_style_1('宋体', 205, False) if a == 4 or a== 8 else set_style_1('Times New Roman', 205, False))
                    ...
                    workboot.save("/media/tx-eva-data/Data1/CFDA_file/数据脱敏/split_table/%s.xls" % date_i)
                else:
                    # 表示某一天有多条数据了

                    read_work = xlrd.open_workbook("/media/tx-eva-data/Data1/CFDA_file/数据脱敏/split_table/%s.xls" % date_i)
                    ...
                    for a in range(0, len(table.row_values(i))):
                        if a == 1:
                            ...
                            sheet2.write(4+list_date[date_i], a, datetime.datetime(*xldate_as_tuple(table.row_values(i)[a], 0)).strftime('%Y-%m-%d') if a == 3 else table.row_values(i)[a], set_style_1('宋体', 205, False) if a == 4 or a== 8 else set_style_1('Times New Roman', 205, False))
                    ...
                    workboot1.save("/media/tx-eva-data/Data1/CFDA_file/数据脱敏/split_table/%s.xls" % date_i)
