...
def split_excel():
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/Data1/CFDA_file/数据标注/segmention_train.xlsx"
    ...
    for row_index in range(rows):
        if row_index == 0:
            ...
            new_clo = ['日期', '数据类型', 'PID', '标注医生1', '标注医生2', '二合一结果', '仲裁医生', '仲裁时间', '仲裁状态']  # 新表格的列的title
            ...
            for a in range(0, len(list_row_data)):
                for b in range(0, len(list_row_data[a])):
                    sheet2.write(a + 1, b, list_row_data[a][b], set_style_1('Time New Roman', 205,
                        ...
                        '宋体', 205, False))
            workboot.save('/media/tx-eva-data/Data1/CFDA_file/数据标注/segmention/split-train/%s.xls' % file_name)
...
def get_markData():
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/NAS/标注数据库/交付数据/segmention/train"
