...
def write_excel():
    ...
    list_3 =[]
    list_date = {}
    data = xlrd.open_workbook("/media/tx-eva-data/Data1/CFDA_file/整改文件/2019-10-17-数据清洗记录.xlsx")
    # 获取每一个sheet的name?
        ...
        if i == 0:
            ...
            if re.match('^\d\-*', date_i):
                if date_i not in list_date:
                    ...
                    list_date[date_i] = 1
                    #  开始写入数据 前面4行书表头
                    sheet1.write_merge(0, 0, 0, 7, ''.join(list_0).strip(), set_style_1('宋体', 400, False))
                    sheet1.write_merge(1, 1, 0, 7, ''.join(list_1).strip(), set_style_2('Time New Roman', 205, False))
                    sheet1.write_merge(2, 2, 0, 7, ''.join(list_2).strip(), set_style_1('宋体', 205, False))
                    for a in range(len(list_3)):
                        print(list_3)
                        sheet1.write(3, a, list_3[a].strip(), set_style_1('宋体', 205, False))
                    for a in range(len(table.row_values(i))):
                        if a == 1:
                            hosp_dict_id = get_hosp_dict()
                            print(hosp_dict_id[table.row_values(i)[a]])
                            sheet1.write(4, a, hosp_dict_id[table.row_values(i)[a]], set_style_1('Time New Roman', 205, False))
                        else:
                            sheet1.write(4, a, datetime.datetime(*xldate_as_tuple(table.row_values(i)[a], 0)).strftime('%Y-%m-%d') if a == 2 else  table.row_values(i)[a], set_style_1('宋体', 205, False) if a ==4 or a== 7 else set_style_1('Times New Roman', 205, False))

                    sheet1.write(10, 3, '记录人签字', set_style_3('宋体', 205, False))
                    sheet1.write(10, 5, '日期', set_style_3('宋体', 205, False))

                    sheet1.write(12, 3, '复核人签字', set_style_3('宋体', 205, False))
                    sheet1.write(12, 5, '日期', set_style_3('宋体', 205, False))
                    ...
                    workboot.save("/media/tx-eva-data/Data1/CFDA_file/整改文件/数据清洗/3/%s.xls" % date_i)
                else:
                    # 表示某一天有多条数据了

                    read_work = xlrd.open_workbook("/media/tx-eva-data/Data1/CFDA_file/整改文件/数据清洗/3/%s.xls" % date_i)
                    ...
                    for ii in range(xlrd_table.nrows):
                        if ii <=3:
                            if ii == 0:
                                sheet2.write_merge(0, 0, 0, 7, ''.join(xlrd_table.row_values(ii)).strip(),
                                                   set_style_1('宋体', 225, False))
                            elif ii == 1:
                                sheet2.write_merge(1, 1, 0, 7, ''.join(xlrd_table.row_values(ii)).strip(),
                                                   set_style_2('Times New Roman', 205, False))
                            elif ii == 2:
                                sheet2.write_merge(2, 2, 0, 7, ''.join(xlrd_table.row_values(ii)).strip(),
                                                   set_style_1('宋体', 205, False))
                            else:
                                for a in range(len(xlrd_table.row_values(ii))):
                    ...
                    for a in range(len(table.row_values(i))):
                        if a == 1:
                            ...
                            sheet2.write(4+list_date[date_i], a, datetime.datetime(*xldate_as_tuple(table.row_values(i)[a], 0)).strftime('%Y-%m-%d') if a == 2 else table.row_values(i)[a], set_style_1('宋体', 205, False) if a ==4 or a== 7 else set_style_1('Times New Roman', 205, False))
                    list_date[date_i] = list_date[date_i] +1 # 对应的日期重复次数加1

                    sheet2.write(10, 3, '记录人签字: ', set_style_3('宋体', 205, False))
                    sheet2.write(10, 5, '日期: ', set_style_3('宋体', 205, False))

                    sheet2.write(12, 3, '审核人签字: ', set_style_3('宋体', 205, False))
                    sheet2.write(12, 5, '日期: ', set_style_3('宋体', 205, False))
                    ...
                    for i in range(0, 8):
                        if i == 7:
                            ...
                            first_col = sheet2.col(i)  # xlwt中是行和列都是从0开始计算的
                            first_col.width = 256 * 12
                    workboot1.save("/media/tx-eva-data/Data1/CFDA_file/整改文件/数据清洗/3/%s.xls" % date_i)
