...
def split_excel():
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/Data1/CFDA_file/数据标注/detection_test.xlsx"
            ...
            for a in range(0, len(list_row_data)):
                for b in range(0, len(list_row_data[a])):
                    sheet2.write(a + 1, b, list_row_data[a][b], set_style_1('Time New Roman', 205,
                        ...
                        '宋体', 205, False))
            workboot.save('/media/tx-eva-data/Data1/CFDA_file/数据标注/detection/split_test_new/%s.xls' % file_name)
...
def get_pid(mark_time, mark_number, mark_project, hospital_name):
    ...
    if len(list(pici_dict.keys())) > 1:
        key_0 = sorted(list(pici_dict.keys()))[0]
        # 先从第一个批次里面拿如果不够的话在到第二个批次里面拿
        list_b = pici_dict[key_0]
        for kkey in list_a:
            if kkey in list_b:
                list_pid.append(kkey)  # 保存的是对应批次的标注量的数据

        for ab in list_pid:
            if ab in list_old_pid:
                list_pid.remove(ab)  # 把已经使用过的id去除掉

        if int(mark_number) > len(list_pid):
            # 说明需要从第二批里面去取数据
            # 清空list_pid
            key_1 = sorted(list(pici_dict.keys()))[1]
            list_c = pici_dict[key_1]
            for ckey in list_a:
                if ckey in list_c:
                    list_pid.append(ckey)

            for abc in list_pid:
                if abc in list_old_pid:
                    list_pid.remove(abc)
            if int(mark_number) > len(list_pid):
                key_3 = sorted(list(pici_dict.keys()))[2]
                list_d = pici_dict[key_3]

                for fkey in list_a:
                    if fkey in list_d:
                        list_pid.append(fkey)
                for gkey in list_pid:
                    if gkey in list_old_pid:
                        list_pid.remove(gkey)

                new_list = list_pid[:int(mark_number)]
                for key_2 in new_list:
                    if key_2 not in list_old_pid:
        ...
        # 说明只有一个批次
        key_0 = list(pici_dict.keys())[0]
        list_d = pici_dict[key_0]
        for aakey in list_a:
            if aakey in list_d:
                list_pid.append(aakey)  # 记录当前批次的标注pid
        # 把已经使用的id要去除掉
        for dkey in list_pid:
            if dkey in list_old_pid:
                list_pid.remove(dkey)

        new_list = list_pid[:int(mark_number)]

        # 把新的pid添加到使用记录里面
        for ekey in new_list:
            if ekey not in list_old_pid:
                list_old_pid.append(ekey)
        return new_list
...
def get_markData():
    '''
    该方法先是获取到对应项目的标注数据的 这里是detection的test
    :return:
    '''
    src_path = "/media/tx-eva-data/NAS/new_addData/备份_不要删/detection"
