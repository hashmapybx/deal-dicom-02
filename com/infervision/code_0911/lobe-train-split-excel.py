...
def split_excel():
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/Data1/CFDA_file/tmp/lobe_train.xlsx"
    data = xlrd.open_workbook(src_path)
    sheet1 = data.sheets()[0]
    rows = sheet1.nrows
    print(rows)
    # list_audit_doctor = [] # 审核医生名字
    list_audit_result = ['通过', '通过', '通过', '通过', '意见不一致，需要仲裁']  # 二合一的结果是随机给但是要保证需要仲裁的结果尽量多一点
    ...
    for row_index in range(rows):
        if row_index == 0:
            ...
            new_clo = ['日期', '标注医生', '标注项目', '数据类型', 'PID', '可用状态','审核医生', '审核结果', '仲裁医生', '仲裁时间', '仲裁状态']  # 新表格的列的title
            ...
            auditor_doctor = sheet1.row_values(row_index)[10]
            ...
            if annotation_number == 0:
                for pid_index, pid_value in enumerate(marked_number_list_new):
                    ...
                    list_row_data.append([mark_time, mark_doctor,mark_project,data_type, pid_value, '已标注', auditor_doctor, a_res,
                                          '' if a_res == '通过' else arbitrate_doctor,
                                          '' if a_res == '通过' else arbitrate_time,
                                          '' if a_res == '通过' else '仲裁完成'])
            # else:
            #     # TODO 表示是有注释量的情况下
            #     for pid_index, pid_value in enumerate(marked_number_list_new):
            #         a_res = list_audit_result[random.randint(0, 4)]  # 随机每一行产生的二合一的结果操作
            #         list_row_data.append([mark_time, data_type, pid_value, mark_doctor,  a_res,
            #                               '' if a_res == '通过' else arbitrate_doctor,
            ...
            #             [mark_time, mark_doctor,mark_project,data_type, b,auditor_doctor, arbitrate_time,
            #              '数据不可用'])
            ...
            for a in range(0, len(list_row_data)):
                for b in range(0, len(list_row_data[a])):
                    sheet2.write(a + 1, b, list_row_data[a][b], set_style_1('Time New Roman', 205,
                        ...
                        '宋体', 205, False))
            workboot.save('/media/tx-eva-data/Data1/CFDA_file/数据标注/lobe/split_train_new_b/%s.xls' % file_name)
...
def get_markData():
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/NAS/标注数据库/交付数据/lobe/train"
    list_pid_a = []  # lobe train第一批的pid
    list_pid_b = []  # lobe train 第二批的pid

    for pici in os.listdir(src_path):  #
        if pici == '2018_01_15':
...
def get_pid(mark_time, mark_project, hospital_name):
    ...
    list_a, list_b  = get_markData()
    for pici in os.listdir(src_path):  # 2.0 2.1
        # TODO 根据detection test 里面 标注的时间在对应的批次之内获取数据 20171116 20180115
        if int(str(mark_time).replace('-', '')) <= 20180115:
            # 找对应医院的数据pid
            for key in list_a:
