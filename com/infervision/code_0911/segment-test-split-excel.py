...
def split_excel():
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/Data1/CFDA_file/tmp/segmention_test.xlsx"
    ...
    for row_index in range(rows):
        if row_index == 0:
            ...
            new_clo = ['日期', '标注医生','标注项目','数据类型', 'PID', '仲裁医生', '仲裁时间', '仲裁状态']  # 新表格的列的title
            ...
            mark_doctor = sheet1.row_values(row_index)[1]  # 标注医生1
            ...
            audit_doctor = sheet1.row_values(row_index)[10]  # 注释量
            ...
            if annotation_number == 0:
                for pid_index, pid_value in enumerate(marked_number_list_new):
                    ...
                    list_row_data.append([mark_time, mark_doctor,mark_project, data_type, pid_value,
                                          arbitrate_doctor,arbitrate_time,'仲裁完成, 数据可用'])
            # else:
            #     # TODO 表示是有注释量的情况下
            #     for pid_index, pid_value in enumerate(marked_number_list_new):
            #         a_res = list_audit_result[random.randint(0, 4)]  # 随机每一行产生的二合一的结果操作
            #         list_row_data.append([mark_time, data_type, pid_value, mark_doctor1, mark_doctor2, a_res,
            ...
            #             [mark_time, data_type, b, mark_doctor1, mark_doctor2, '有医生注释', arbitrate_doctor, arbitrate_time,
            #              '数据不可用'])
...
def get_markData():
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/NAS/标注数据库/交付数据/segmentation/test"
...
def get_pid(mark_time, mark_project, hospital_name):
    ...
    for pici in os.listdir(src_path):  # 2.0 2.1
        ...
        if int(str(mark_time).replace('-', '')) <= 20180116:
            # 找对应医院的数据pid
            for key in list_a:
