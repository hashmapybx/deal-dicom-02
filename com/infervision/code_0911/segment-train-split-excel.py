...
def split_excel():
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/Data1/CFDA_file/tmp/segmention_train.xlsx"
    ...
    for row_index in range(rows):
        if row_index == 0:
            ...
            new_clo = ['日期', '标注医生', '标注项目', '数据类型', 'PID', '审核医生', '审核结果', '仲裁医生', '仲裁时间', '仲裁状态']  # 新表格的列的title
            ...
            for ppid in marked_number_list_old:
                if ppid in pid_list:
                    pid_list.remove(ppid)
                print(pid_list)
            # 取出来当前医生的标注量
            ...
            if annotation_number == 0:
                for pid_index, pid_value in enumerate(marked_number_list_new):
                    ...
                    list_row_data.append([mark_time, mark_doctor,mark_project,data_type, pid_value, auditor_doctor, a_res,
            ...
            for a in range(0, len(list_row_data)):
                for b in range(0, len(list_row_data[a])):
                    sheet2.write(a + 1, b, list_row_data[a][b], set_style_1('Time New Roman', 205,
                        ...
                        '宋体', 205, False))
            workboot.save('/home/tx-eva-data/Downloads/CFDA文件_0922/new/12数据标注/segmention/tmp/%s.xls' % file_name)
...
def get_markData():
    ...
    :return:
    '''
    src_path = "/media/tx-eva-data/NAS/标注数据库/交付数据/segmentation/train"
...
def get_pid(mark_time, mark_project,hospital_name):
    ...
    a_path = "/media/tx-eva-data/NAS/标注数据库/交付数据/segmentation/train"
    new_list = []  # 保存传进来对应的医院的pid
    list_a = get_markData()
    for number in os.listdir(a_path):  # 2.0 2.1
